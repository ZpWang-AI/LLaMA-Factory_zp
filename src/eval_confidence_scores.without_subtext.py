...
class ConfidenceScoresEvaluator:
    ...
    def eval(
        ...
        output_string = []
        threshold_lst = []  
        # fig, ax_list = plt.subplots(nrows=1,ncols=1,figsize=(8,4.8*4))
        # plt.subplots_adjust(hspace=0.5)
        make_path(result_dir)
        plt.figure(figsize=(10, 6))
        plt.rcParams['font.size'] = fontsize
        for lid, label in enumerate(self.label_list):
            score_lst, acc_lst = self.cal_target_acc_woS(lid)
            ...
            if draw:
                plt.xlabel(xlabel)
                plt.ylabel(ylabel)
                plt.savefig(result_dir/f'{label}.{png_name}', bbox_inches='tight')
                plt.clf()
    ...
    def cal_target_acc(self, target):
        ...
        # acc_lst = [(p+acc_init) for p in acc_lst]
        return score_lst, acc_lst
    
    def cal_target_acc_woS(self, target):
        score_lst, acc_lst = [], []
        acc_cnt = 0
        tot_cnt = 0
        # acc_init = 0
        for score, score_pred, rest_pred, gt in self.sorted_samples[::-1]:
            if score_pred != target:
                continue
            score_lst.append(score)
            if score_pred == gt:
                acc_cnt += 1
            tot_cnt += 1
            acc_lst.append(acc_cnt/tot_cnt)
        # score_lst
        # acc_lst = [(p+acc_init) for p in acc_lst]
        start_sample = [10,20,50,10][target]
        return score_lst[start_sample:], acc_lst[start_sample:]
...
if __name__ == '__main__':
    # # pdtb2
    # score_dir = '/home/user/test/zpwang/LLaMA/exp_space/paper_needed/main/pdtb2-top/2024-08-31-11-56-47.main_base_Ptrain.ckpt6000'
    # rest_dir = '/home/user/test/zpwang/LLaMA/exp_space/paper_needed/main/pdtb2-top/2024-08-31-12-02-08.main_subtext_Ptrain.ckpt6000'
    # test_score_dir = '/home/user/test/zpwang/LLaMA/exp_space/paper_needed/main/pdtb2-top/2024-08-31-10-25-30.main_base.ckpt6000'
    # test_rest_dir = '/home/user/test/zpwang/LLaMA/exp_space/paper_needed/main/pdtb2-top/2024-08-31-10-34-07.main_subtext.ckpt6000'

    # pdtb3
    score_dir = '/home/user/test/zpwang/LLaMA/exp_space/paper_needed/main/pdtb3-top/2024-07-12-11-43-25.main_base_dev.ckpt7000.bs1*8_lr0.0001_ep5'
    ...
    rest_dir = '/home/user/test/zpwang/LLaMA/exp_space/paper_needed/main/pdtb3-top/all_false_dev'
    ...
    whichfirst = 'baseFirst'
    # whichfirst = 'subtextFirst'
    df_split = 'dev'
    split_piece = None
    extra_info = '_withoutS'
    ...
    elif data_name == 'pdtb2':
        data_path = '/home/user/test/zpwang/IDRR_data/data/dataBuild/pdtb2_top_implicit.subtext_distill.llama_distill_all.csv'
    else:
        raise ''
    if whichfirst == 'subtextFirst':
        score_dir,rest_dir,test_score_dir,test_rest_dir = (
            rest_dir,score_dir,test_rest_dir,test_score_dir
        )
    result_dir = path(
        f'/home/user/test/zpwang/LLaMA/exp_space/paper_needed/main/{data_name}-{data_level}',
        f'{whichfirst}_{df_split}_{split_piece}{extra_info}'
    )
    
    score_evalor = ConfidenceScoresEvaluator(
        dfs=IDRRDataFrames(
            data_name=data_name, 
            data_level=data_level, data_relation=data_relation,
            data_path=data_path
        ),
        split=df_split,
        score_dir=score_dir,
    ...
    score_evalor.eval(
        ...
        png_name=f'{df_split}_score.png',
        xlabel='Threshold of Confidence Score', ylabel='acc',
        # title=df_split,
